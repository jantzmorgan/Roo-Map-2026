...
const STORAGE_KEYS = {
	...
	plannerPickFilter: "roo_planner_pick_filter_v1",
	plannerMapFilter: "roo_planner_map_filter_v1",
...
const appState = {
	...
	plannerPickFilter: loadValue(STORAGE_KEYS.plannerPickFilter, "All"),
	plannerMapFilter: loadValue(STORAGE_KEYS.plannerMapFilter, "All"),
...
if (!["All", "must", "maybe", "skip"].includes(appState.plannerPickFilter)) appState.plannerPickFilter = "All";
if (!["All", "centeroo", "outeroo"].includes(appState.plannerMapFilter)) appState.plannerMapFilter = "All";
...
function bootRooApp() {
	...
	*/
	...
	bindRooUpdateModalControls();
	showRooUpdateDoneIfNeeded();
...
function togglePick(eventId, pickType) {
	const openSheet = document.querySelector("[data-bottom-sheet].is-open");
	const sheetTitle = document.querySelector("[data-sheet-title]");
	const isTimelineGapPick = Boolean(
		openSheet &&
		sheetTitle &&
		sheetTitle.textContent.trim() === "Fill Open Gap"
	);
...
function bindPlannerFilterControls() {
	document.addEventListener("change", (event) => {
		if (event.target.matches("[data-planner-pick-filter]")) {
			appState.plannerPickFilter = event.target.value || "All";
			saveValue(STORAGE_KEYS.plannerPickFilter, appState.plannerPickFilter);
			renderPlanner();
		}

		if (event.target.matches("[data-planner-map-filter]")) {
			appState.plannerMapFilter = event.target.value || "All";
			saveValue(STORAGE_KEYS.plannerMapFilter, appState.plannerMapFilter);
			renderPlanner();
		}
	});
}
...
function syncPlannerFilterControls() {
	const pickSelect = document.querySelector("[data-planner-pick-filter]");
	const mapSelect = document.querySelector("[data-planner-map-filter]");

	if (pickSelect) {
		pickSelect.value = appState.plannerPickFilter || "All";
	}

	if (mapSelect) {
		mapSelect.value = appState.plannerMapFilter || "All";
	}
}

function filterPlannerEvents(events) {
	return events.filter((event) => {
		const matchesPick =
			appState.plannerPickFilter === "All" ||
			event.pickType === appState.plannerPickFilter;

		const matchesMap =
			appState.plannerMapFilter === "All" ||
			event.mapId === appState.plannerMapFilter;

		return matchesPick && matchesMap;
	});
}

function getPlannerFilterLabel() {
	const parts = [];

	if (appState.plannerDay && appState.plannerDay !== "All") {
		parts.push(appState.plannerDay);
	}

	if (appState.plannerPickFilter && appState.plannerPickFilter !== "All") {
		parts.push(getPickLabel(appState.plannerPickFilter));
	}

	if (appState.plannerMapFilter && appState.plannerMapFilter !== "All") {
		parts.push(appState.plannerMapFilter === "centeroo" ? "Centeroo" : "Outeroo");
	}
...
function renderPlanner() {
	...
	const allPickedForDay = filterEventsByDay(getPickedEvents(true), appState.plannerDay);
	const picked = filterPlannerEvents(allPickedForDay);
	...
	if (picked.length === 0) {
		list.innerHTML = `
			<div class="empty-state planner-empty-state">
				<strong>♪ No picks match ${escapeHTML(getPlannerFilterLabel())}.</strong>
				<span>Change the Planner filters or tap a stage/search result to mark acts as Must, Maybe, or Skip.</span>
			</div>
		`;
		return;
	}
	...
	groups.forEach((group) => {
		...
		groupEvents.forEach((event) => {
			...
			const dayClass = getDayClass(event.day);
			const mapLabel = event.mapId === "outeroo" ? "Outeroo" : "Centeroo";
			...
			card.innerHTML = `
				<div>
					...
					<h3>${escapeHTML(event.artist)}</h3>
					<p class="soft-text">${escapeHTML(event.placeName)} · ${escapeHTML(mapLabel)}</p>
					${conflict ? `<div class="conflict-note">${escapeHTML(conflict)}</div>` : ""}
...
/* =========================================================
   FRIEND-PROOF SAFE UPDATE FLOW
========================================================= */

let rooUpdateModalBound = false;
let rooUpdateReloading = false;
let rooUpdateRegistration = null;

const ROO_SAFE_UPDATE_KEYS = {
	latestBackup: "roo_safe_update_latest_backup_v1",
	latestBackupTime: "roo_safe_update_latest_backup_time_v1",
	expectedPickCount: "roo_update_expected_pick_count_v1",
	expectedSpotCount: "roo_update_expected_spot_count_v1",
	donePending: "roo_update_done_pending_v1",
	autoRestored: "roo_update_auto_restored_v1"
};

function bindRooUpdateModalControls() {
	if (rooUpdateModalBound) return;
	rooUpdateModalBound = true;

	document.addEventListener("click", (event) => {
		const button = event.target.closest("[data-update-roo-map]");
		if (!button) return;
...
function countRooObjectItems(value) {
	if (!value || typeof value !== "object" || Array.isArray(value)) return 0;
	return Object.keys(value).length;
}

function countRooArrayItems(value) {
	return Array.isArray(value) ? value.length : 0;
}

function safeParseRooJSON(value, fallback) {
	try {
...
function getRooBackupPicksFromStorage(storage) {
	if (!storage || !storage[STORAGE_KEYS.picks]) return {};
	return safeParseRooJSON(storage[STORAGE_KEYS.picks], {});
}

function getRooBackupSpotsFromStorage(storage) {
	if (!storage || !storage[STORAGE_KEYS.savedSpots]) return [];
	return safeParseRooJSON(storage[STORAGE_KEYS.savedSpots], []);
}

function createRooSafeUpdateBackup(reason = "safe-update") {
	try {
		if (typeof savePicksBackup === "function") {
			savePicksBackup(reason);
		}

		const currentPicks = appState && appState.picks ? appState.picks : loadJSON(STORAGE_KEYS.picks, {});
		const currentSpots = appState && Array.isArray(appState.savedSpots) ? appState.savedSpots : loadJSON(STORAGE_KEYS.savedSpots, []);
		...
		const backup = {
			...
			expectedPickCount: countRooObjectItems(currentPicks),
			expectedSpotCount: countRooArrayItems(currentSpots),
			storage: {}
		};
		...
		backup.storage[STORAGE_KEYS.picks] = JSON.stringify(currentPicks || {});
		backup.storage[STORAGE_KEYS.savedSpots] = JSON.stringify(currentSpots || []);
		backup.storage[STORAGE_KEYS.amenityFilters] = JSON.stringify(appState.amenityFilters || DEFAULT_AMENITY_FILTERS);
		backup.storage[STORAGE_KEYS.settings] = JSON.stringify(appState.settings || DEFAULT_SETTINGS);
		backup.storage[STORAGE_KEYS.calibrationByMap] = JSON.stringify(appState.calibrationByMap || {});

		localStorage.setItem(ROO_SAFE_UPDATE_KEYS.latestBackup, JSON.stringify(backup));
		localStorage.setItem(ROO_SAFE_UPDATE_KEYS.latestBackupTime, backup.savedAt);
		localStorage.setItem(ROO_SAFE_UPDATE_KEYS.expectedPickCount, String(backup.expectedPickCount));
		localStorage.setItem(ROO_SAFE_UPDATE_KEYS.expectedSpotCount, String(backup.expectedSpotCount));

		return backup;
	} catch (error) {
		console.warn("Safe update backup failed:", error);
		return null;
	}
}

function getRooSafeUpdateBackup() {
	try {
		return JSON.parse(localStorage.getItem(ROO_SAFE_UPDATE_KEYS.latestBackup) || "null");
	} catch {
		return null;
	}
}

function restoreRooDataFromSafeUpdateBackup(backup) {
	if (!backup || !backup.storage) return false;

	const keysToRestore = [
		STORAGE_KEYS.picks,
		STORAGE_KEYS.savedSpots,
		STORAGE_KEYS.amenityFilters,
		STORAGE_KEYS.settings,
		STORAGE_KEYS.calibrationByMap,
		STORAGE_KEYS.plannerDay,
		STORAGE_KEYS.plannerPickFilter,
		STORAGE_KEYS.plannerMapFilter,
		STORAGE_KEYS.timelineDay,
		STORAGE_KEYS.selectedDay
	];

	keysToRestore.forEach((key) => {
		if (backup.storage[key] !== null && backup.storage[key] !== undefined) {
			localStorage.setItem(key, backup.storage[key]);
		}
	});

	appState.picks = loadJSON(STORAGE_KEYS.picks, {});
	appState.savedSpots = loadJSON(STORAGE_KEYS.savedSpots, []);
	appState.amenityFilters = {
		...DEFAULT_AMENITY_FILTERS,
		...loadJSON(STORAGE_KEYS.amenityFilters, {})
	};
	appState.settings = {
	...
	appState.calibrationByMap = normalizeCalibrationStore(
		loadJSON(STORAGE_KEYS.calibrationByMap, null)
	);

	appState.selectedDay = loadValue(STORAGE_KEYS.selectedDay, "All");
	appState.plannerDay = loadValue(STORAGE_KEYS.plannerDay, "All");
	appState.plannerPickFilter = loadValue(STORAGE_KEYS.plannerPickFilter, "All");
	appState.plannerMapFilter = loadValue(STORAGE_KEYS.plannerMapFilter, "All");
	appState.timelineDay = loadValue(STORAGE_KEYS.timelineDay, "All");
...
function recoverRooDataAfterUpdateIfNeeded() {
	const expectedPickCount = Number(localStorage.getItem(ROO_SAFE_UPDATE_KEYS.expectedPickCount) || "0");
	const currentPicks = loadJSON(STORAGE_KEYS.picks, {});
	const currentPickCount = countRooObjectItems(currentPicks);

	if (expectedPickCount <= 0) return "normal";
	if (currentPickCount > 0) return "normal";

	const safeBackup = getRooSafeUpdateBackup();
	const safeBackupPicks = safeBackup ? getRooBackupPicksFromStorage(safeBackup.storage) : {};

	if (countRooObjectItems(safeBackupPicks) > 0) {
		restoreRooDataFromSafeUpdateBackup(safeBackup);
		localStorage.setItem(ROO_SAFE_UPDATE_KEYS.autoRestored, "yes");
		return "restored";
	}

	if (typeof getBestBackup === "function") {
		const bestBackup = getBestBackup();

		if (bestBackup && countRooObjectItems(bestBackup.picks) > 0) {
			appState.picks = bestBackup.picks;
			saveJSON(STORAGE_KEYS.picks, appState.picks);
			savePicksBackup("auto-restore-after-update");
...
function showRooUpdateReady() {
	createRooSafeUpdateBackup("update-ready");

	const modal = document.querySelector("[data-update-ready-modal]");
	const status = document.querySelector("[data-update-modal-status]");
	const button = document.querySelector("[data-update-roo-map]");
	...
	modal.classList.remove("is-updating");
	modal.classList.remove("is-done");
	modal.classList.add("is-open");
	modal.setAttribute("aria-hidden", "false");

	if (status) {
		status.textContent = "Your picks are backed up. Tap update. Don’t delete the app.";
	}

	button.disabled = false;
	button.textContent = "Update Roo Map";
}

function updateRooMapNow() {
	if (rooUpdateReloading) return;
	rooUpdateReloading = true;

	createRooSafeUpdateBackup("before-update-button");

	const modal = document.querySelector("[data-update-ready-modal]");
	const status = document.querySelector("[data-update-modal-status]");
	const button = document.querySelector("[data-update-roo-map]");
	const icon = modal ? modal.querySelector(".update-ready-icon") : null;

	if (modal) {
		modal.classList.remove("hidden");
		modal.classList.add("is-open");
		modal.classList.add("is-updating");
		modal.setAttribute("aria-hidden", "false");
	}
	...
	if (status) {
		status.textContent = "Updating Roo Map…";
	}
	...
	localStorage.setItem(ROO_SAFE_UPDATE_KEYS.donePending, "yes");

	if (
		rooUpdateRegistration &&
		rooUpdateRegistration.waiting &&
		rooUpdateRegistration.waiting.postMessage
	) {
		try {
			rooUpdateRegistration.waiting.postMessage({ type: "SKIP_WAITING" });
		} catch (error) {
			console.warn("Could not message waiting service worker:", error);
		}
	}
...
function showRooUpdateDoneIfNeeded() {
	if (localStorage.getItem(ROO_SAFE_UPDATE_KEYS.donePending) !== "yes") return;

	localStorage.removeItem(ROO_SAFE_UPDATE_KEYS.donePending);

	window.setTimeout(() => {
		const recoveryStatus = recoverRooDataAfterUpdateIfNeeded();
		...
		if (!modal) {
			showToast(recoveryStatus === "restored" ? "Update done ✅ Picks restored" : "Update done ✅");
			return;
		}
		...
		if (status) {
			if (recoveryStatus === "restored") {
				status.textContent = "Update done ✅ Your picks were restored.";
			} else if (recoveryStatus === "missing") {
				status.textContent = "Update done. Import backup if picks are missing.";
			} else {
				status.textContent = "Update done ✅ Your picks are still here.";
			}
		}
...
function registerServiceWorker() {
	...
	window.addEventListener("load", () => {
		navigator.serviceWorker.register("service-worker.js")
			.then((registration) => {
				rooUpdateRegistration = registration;
				...
				if (registration.update) {
					registration.update().catch(() => {
						/* Ignore weak signal update checks. */
					});
				}
			})
			.catch((error) => {
