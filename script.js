...
function createRooSafeUpdateBackup(reason = "safe-update") {
	try {
		...
		localStorage.setItem("roo_safe_update_latest_backup_v1", JSON.stringify(backup));
		localStorage.setItem("roo_safe_update_latest_backup_time_v1", backup.savedAt);
...
function updateRooMapNow() {
	...
	localStorage.setItem("roo_update_done_pending_v1", "yes");
...
function showRooUpdateDoneIfNeeded() {
	if (localStorage.getItem("roo_update_done_pending_v1") !== "yes") return;

	localStorage.removeItem("roo_update_done_pending_v1");
