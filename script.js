...
function bootRooApp() {
	...
	setActiveScreen(appState.activeScreen, { skipRender: true });
	setActiveMap(appState.activeMapId, { skipSaveCurrentTransform: true, skipToast: true });
...
function setActiveMap(mapId, options = {}) {
	...
	if (!options.skipSaveCurrentTransform) {
		saveMapTransform();
	}
...
function getGapNote(previous, current) {
	if (!previous || !current || previous.day !== current.day) return "";

	const previousEnd = normalizeEndMinutes(previous.start, previous.end);
	const currentStart = timeToMinutes(current.start);
	const walk = getWalkTime(previous.venueId, current.venueId) || 0;
	const freeMinutes = currentStart - previousEnd - walk;

	if (freeMinutes < 25) return "";

	return `Open gap · ${freeMinutes} min before ${current.artist}`;
}
...
function renderTimeline() {
	...
	picked.forEach((event, index) => {
		...
		const walkNote = previous ? getWalkNote(previous, event) : "";
		...
		item.innerHTML = `
			...
			<div class="timeline-content">
				${walkNote ? `<div class="timeline-walk">${escapeHTML(walkNote)}</div>` : ""}
				${gapNote ? `<div class="timeline-gap">${escapeHTML(gapNote)}</div>` : ""}
				<h3>${escapeHTML(event.artist)}</h3>
...
/* =========================================================
   ROO UX UPGRADE PATCH — 20260604
   Loading screen, map switch loading, welcome popup,
   backup status, safer warnings, share button,
   offline/update messages, GPS text, Today at Roo.
========================================================= */
...
(() => {
	...
	let rooMapLoadingTimer = null;
	let rooAppLoadingTimer = null;

	/* =========================
	   STARTUP LOADING
	========================= */

	showRooLoading("Loading Roo Map…");
	...
	function showRooLoading(text = "Loading Roo Map…") {
		...
		clearTimeout(rooAppLoadingTimer);
	...
	function hideRooLoading() {
		...
		rooAppLoadingTimer = setTimeout(() => {
			overlay.classList.add("hidden");
			overlay.classList.remove("is-hiding");
		}, 240);
	}

	function setRooMapLoading(label) {
		const viewport = document.querySelector("[data-map-viewport]");
		if (viewport) {
		...
		clearTimeout(rooMapLoadingTimer);

		rooMapLoadingTimer = setTimeout(() => {
			clearRooMapLoading();
		}, 1800);
	}

	function clearRooMapLoading() {
		clearTimeout(rooMapLoadingTimer);
		rooMapLoadingTimer = null;
		document.body.classList.remove("is-map-loading");
	}

	function finishRooLoadingSoon() {
	...
	if (typeof renderMapShell === "function") {
		const originalRenderMapShell = renderMapShell;

		renderMapShell = function renderMapShellWithLoading() {
			const map = getActiveMap ? getActiveMap() : null;

			if (map && map.name) {
				setRooMapLoading(`Loading ${map.name}…`);
			}

			const result = originalRenderMapShell.apply(this, arguments);

			const image = document.querySelector("[data-map-image]");
			if (!image) {
				finishRooLoadingSoon();
				return result;
			}
	...
	if (typeof setActiveMap === "function") {
		const originalSetActiveMap = setActiveMap;

		setActiveMap = function setActiveMapWithLoading(mapId, options = {}) {
			const map = typeof getMap === "function" ? getMap(mapId) : null;

			if (map && map.name && !options.skipMapLoading) {
				setRooMapLoading(`Loading ${map.name}…`);
			}

			return originalSetActiveMap.apply(this, arguments);
		};
	}

	/* =========================
	   DOM READY UX BINDINGS
	========================= */

	runWhenReady(() => {
		bindRooWelcomeControls();
		bindRooShareButton();
		bindRooServiceWorkerMessages();
		startRooAnchorLabelWatcher();
	...
	function bindRooWelcomeControls() {
		document.addEventListener("click", (event) => {
	...
	function normalizeGPSAnchorLabels() {
		document.querySelectorAll("[data-save-place-anchor], [data-save-amenity-anchor]").forEach((button) => {
			button.textContent = "GPS Anchor";
		});

		document.querySelectorAll(".empty-state span").forEach((span) => {
			span.textContent = span.textContent.replace("tap Anchor again", "tap GPS Anchor again");
		});
	}

	function startRooAnchorLabelWatcher() {
		normalizeGPSAnchorLabels();

		const app = document.querySelector("[data-roo-app]");
		if (!app) return;
	...
	if (typeof createNowNextCard === "function") {
		createNowNextCard = function createNowNextCardUX(place, events) {
			const card = document.createElement("div");
			card.className = "mini-card mini-card--now-next";

			const sorted = events.slice().sort(sortEventsByDayAndTime);
			const current = getCurrentDayAndMinutes();
			const now = current ? findNowPlaying(sorted) : null;
			const next = current ? findNextEvent(sorted) : null;
			...
			if (!current) {
				html = `
					<div>
						<span>Schedule</span>
						<strong>Festival starts on June 10th, 2026</strong>
						<small>Pick artists below to build your Roo plan.</small>
					</div>
				`;
			} else {
	...
	/* =========================
	   TODAY AT ROO TIMELINE
	========================= */

	if (typeof renderTimeline === "function") {
		renderTimeline = function renderTimelineUX() {
			const list = document.querySelector("[data-timeline-list]");
			if (!list) return;

			const current = getCurrentDayAndMinutes();
			let picked = getPickedEvents(false);

			if (current && current.day) {
				picked = picked.filter((event) => event.day === current.day);
			}

			if (picked.length === 0) {
				list.innerHTML = `
					<div class="empty-state">
						<strong>${current ? "Today at Roo" : "No timeline yet."}</strong>
						<span>${current ? "No Must or Maybe picks for today yet." : "Your saved picks will appear here in time order."}</span>
					</div>
				`;
				return;
			}
			...
			if (current && current.day) {
				...
				todayHeader.className = "timeline-day-header";
				todayHeader.textContent = `Today at Roo · ${current.day}`;
			...
			picked.forEach((event, index) => {
				...
				const walkNote = previous && previous.day === event.day ? getWalkNote(previous, event) : "";

				const item = document.createElement("article");
				item.className = "timeline-item";

				item.innerHTML = `
					<div class="timeline-time">${formatTime(event.start)}</div>

					<div class="timeline-content">
						${walkNote ? `<div class="timeline-walk">${escapeHTML(walkNote)}</div>` : ""}
						<h3>${escapeHTML(event.artist)}</h3>
						<p>${escapeHTML(event.placeName)} · ${event.pickType === "must" ? "Must" : "Maybe"}</p>
					</div>
				`;
	...
	if (typeof enableLocation === "function") {
		enableLocation = function enableLocationUX() {
			if (!navigator.geolocation) {
				showToast("GPS is not available in this browser.");
				return;
			}
	...
	function getFallbackBestPicksBackup() {
		const keys = [
			STORAGE_KEYS.picksBackup,
			STORAGE_KEYS.picksBackupLastGood
		].filter(Boolean);

		const backups = keys
			.map((key) => loadJSON(key, null))
			.filter((backup) => backup && typeof backup === "object" && backup.picks);
	...
	function getUXBestBackup() {
		if (typeof getBestPicksBackup === "function") {
			return getBestPicksBackup();
		}

		return getFallbackBestPicksBackup();
	}
	...
	function maybeShowBackupReminder() {
		const count = typeof getPickCount === "function" ?
			getPickCount(appState.picks) :
			Object.keys(appState.picks || {}).length;

		if (count < 10) return;
		if (localStorage.getItem(UX_KEYS.backupReminderSeen) === "yes") return;

		localStorage.setItem(UX_KEYS.backupReminderSeen, "yes");

		window.setTimeout(() => {
			showToast("Don’t forget to Export Backup after making picks");
		}, 450);
	}
	...
	if (typeof renderSettings === "function") {
		...
		renderSettings = function renderSettingsUX() {
	...
	if (typeof togglePick === "function") {
		const originalTogglePick = togglePick;

		togglePick = function togglePickUX(eventId, pickType) {
			const result = originalTogglePick.apply(this, arguments);
	...
	if (typeof exportRooData === "function") {
		const originalExportRooData = exportRooData;

		exportRooData = function exportRooDataUX() {
			const result = originalExportRooData.apply(this, arguments);
	...
	if (typeof importRooData === "function") {
		const originalImportRooData = importRooData;

		importRooData = function importRooDataUX(file) {
			showToast("Importing backup…");

			const result = originalImportRooData.apply(this, arguments);
	...
	if (typeof resetRooData === "function") {
		resetRooData = function resetRooDataUX() {
			const ok = confirm(
	...
	if (typeof clearPicks === "function") {
		clearPicks = function clearPicksUX() {
			const ok = confirm(
	...
	async function shareRooMapLink() {
	...
	/* =========================
	   OFFLINE + UPDATE READY
	========================= */

	function bindRooServiceWorkerMessages() {
		if (!("serviceWorker" in navigator)) return;
		...
		navigator.serviceWorker.ready
			...
			.catch(() => {
				/* Service worker not ready yet. Ignore quietly. */
			});
	...
	if (typeof applyMapTransform === "function") {
		const originalApplyMapTransform = applyMapTransform;

		applyMapTransform = function applyMapTransformUX() {
			const result = originalApplyMapTransform.apply(this, arguments);
			updateZoomLabel();
			return result;
		};
	}
	...
	function updateZoomLabel() {
		...
		if (!label || !appState.map.fitScale) return;

		const percent = Math.round((appState.map.scale / appState.map.fitScale) * 100);
